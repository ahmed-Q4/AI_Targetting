...
# Non-Micro-Cap
DT6.2 <- dplyr::filter(DT5.2, CAP_GROUP %in% c("SMALL","MID", "MEGA", "LARGE"))

# Focusing on US only positions based on exchange (NYS and NAS)
DT7.2.us <- dplyr::filter(DT6.2, grepl(pattern = "(-NYS)|(-NAS)", x = TICKER_EXCHANGE) )
DT7.2.non_us <- dplyr::filter(DT6.2, !grepl(pattern = "(-NYS)|(-NAS)", x = TICKER_EXCHANGE) )

cleanMem()
# Writing results to database
library(RMySQL)
con <- dbConnect(RMySQL::MySQL(),  default.file = "~/.my.cnf", group = "local_intel")
dbWriteTable(con, "fund_us_holdings_hist_w_symbols_and_sic", DT7.2.us, append = TRUE)
dbWriteTable(con, "fund_non_us_holdings_hist_w_symbols_and_sic", DT7.2.non_us, append = TRUE)
dbDisconnect(con)
# backing up tables to RDS files
saveRDS(DT7.2.us, "own_fund_holdings_hist_1_us_part2.rds")
saveRDS(DT7.2.non_us, "own_fund_holdings_hist_1_non_us_part2.rds")
