...
# Plotting the Y vs X to look at the data -----------
...
# Trying different proposed model/solution
# 1. Bias reduction in binomial-response generalized linear models: brglm package 
tt1 <- brglm(Buy_Sell ~ ., family=binomial(), data = Training_data_classification)
# 2. Baysian GLM
tt2 <- arm::bayesglm(Buy_Sell ~ ., family=binomial(link="probit"), data = Training_data_classification)
summary(tt2)
# 3. Robust GLM from robustbase package
tt3 <- robustbase::glmrob(Buy_Sell ~ ., family=binomial(link="probit"), data = Training_data_classification)
...
# Model Comparaison based on predictions
